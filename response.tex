...
\resp{
    ...
    The reviewer is referred to the response letter to Part I for the details on how we changed the motivation of the paper to avoid the confusion as well as the precision that we added. 
}
...
\resp{
    ...
    In order to measure these inefficiencies, the important output of the day-ahead model is the unit commitment decision and our model does not produce prices explicitly. 
    ...
    Traditionally, the European pricing rules have forbidden the recourse to uplift payments, at the cost of a potential loss in welfare.
    However, these rules are being reconsidered and the market is expected to evolve towards a full welfare maximization approach. 
    We do not address these questions in details in our paper and, instead, assume that units are committed under welfare maximization. 
    This approach leads to a best-case investigation of zonal pricing and enables us to focus on the inefficiencies in unit commitment brought in by simplified transmission constraints. 
    
    Finally, we note that our model could in theory be extended to compute prices under the different pricing mechanisms. 
    In the case of IP pricing, the procedure would be simply to fix the switching and commitment variables to obtain a linear problem. 
    The dual variable of the zonal balance constraint gives the zonal price. 
    ...
    In the case of the current European pricing rules that require strict linear prices, the prices could be obtained by again fixing the switching variables and follow the approach presented in \cite{madani2015} to reformulate the equilibrium problem as a Mixed Integer Problem.
    ...
    In this regard, the footnote n$^\circ$4 on page 3 of our previous submission was probably misleading by suggesting that our model computes prices explicitly. 
    Following the comment of the reviewer, we have modified this footnote. 
    We repeat this modification here for convenience. 
    \begin{quotation}
        {\it
            We assume that the commitment is determined along with the topology \textcolor{red}{by welfare maximization. Our model does not compute prices explicitly but supporting prices could be computed after the binary decisions have been fixed, as in US markets. A full analysis of the implications of our zonal day-ahead model on pricing is however outside the scope of the paper, and is the subject of follow-up research.}
        }
    \end{quotation}
}
