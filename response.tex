...
\resp{
    ...
    The instance with the data, inspired by the small example presented in \cite{Schiro2015}, is shown in Fig. \ref{fig:triangle}. 
    \begin{figure}
        \centering
        \include{triangle}
    ...
    Then, prices will be computed following the rules of each method with the fixed topology. 
    \begin{table}[H]
        \color{blue}
        \centering
    ...
    \footnotetext{{\color{blue}Note that the EU-style price leads to a different dispatch than IP and CHP pricing at a load level of 550MW, which translates in a loss of welfare. Generator $G_2$ is in that case not committed and is 'paradoxically rejected', which is allowed according to EU rules. No side payments are required.}}
    ...
    \begin{quotation}
        {\it
            ...
            Prices can be computed after the binary decisions have been fixed \cite{PJM}. A full analysis of the implications of our zonal day-ahead model on pricing is however outside the scope of the paper, and is the subject of follow-up research.}
        }
    \end{quotation}
    
    \quentin{!!! Is \cite{PJM} the right citation to make here ?}
}
